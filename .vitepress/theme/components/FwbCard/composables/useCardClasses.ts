...
export function useCardsClasses(props: UseCardsClassesProps): {
  ...
  const cardClasses = computed(() => {
    ...
    if (props.variant.value === 'default') {
      computedClasses = 'block bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700'
    } else if (props.variant.value === 'horizontal') {
      computedClasses = 'flex flex-1 flex-col items-center bg-white rounded-lg border shadow-md md:flex-row dark:border-gray-700 dark:bg-gray-800'
    }
  ...
  const horizontalImageClasses = computed(() => (props.variant.value === 'horizontal')
    ? 'object-cover w-full h-26 md:h-96 rounded-t-lg md:h-auto md:w-48 md:rounded-none md:rounded-l-lg'
    : '',
  )
